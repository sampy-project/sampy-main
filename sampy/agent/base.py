...
class BaseAgent:
    """
    Base class for agent. Create the dataframe df_population with a column 'col_id'. 
    Create a 'graph' attribute, containing a Graph object which represents the space on
    which the agents live.
    """
    def __init__(self, graph=None, **kwargs):
        if graph is None:
            raise ValueError("A graph object should be passed to the Agents constructor, using the kwarg 'graph'.")
        self.graph = graph
    ...
    def drop_attribute(self, name_attr):
        """
        Remove a column from the Dataframe df_population, whose name is the parameter 'name_attr'

        :param name_attr: string, name of the attribute to drop
        """
        del self.dict_default_val[name_attr]
        self.df_population.drop_column(name_attr)
    ...
    def add_agents_from_dataframe(self, dataframe: DataFrameXS):
        ...
        dataframe['col_id'] = np.arange(self.counter_id, self.counter_id + dataframe.nb_rows)
    ...
    def shuffle(self):
        """
        Randomly shuffle the agents, i.e. shuffle the lines of df_population.
        """
